...
export interface IQuestionLibrary extends Document {
  ...
  type: 'likert' | 'multiple_choice' | 'open_ended' | 'scale' | 'binary' | 'matrix' | 'emoji_rating';
...
const QuestionLibrarySchema = new Schema<IQuestionLibrary>({
  ...
  type: { 
    type: String, 
    enum: ['likert', 'multiple_choice', 'open_ended', 'scale', 'binary', 'matrix', 'emoji_rating'],
    required: true 
  },
...
QuestionLibrarySchema.methods.createVersion = async function(updates: Partial<IQuestionLibrary>, userId: mongoose.Types.ObjectId) {
