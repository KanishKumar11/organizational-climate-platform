...
async function getMicroclimateData(id: string, session: any) {
  ...
  // Don't use .lean() so we have access to instance methods
  ...
  // Sanitize data to avoid circular references and ensure serializability
  const sanitizedData = {
    id: microclimate._id.toString(),
    title: microclimate.title,
    description: microclimate.description,
    ...
    real_time_settings: {
      ...
      allow_comments: microclimate.real_time_settings?.allow_comments || false,
      word_cloud_enabled:
  ...
  return sanitizeForSerialization(sanitizedData);
}
