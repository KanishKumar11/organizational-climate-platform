...
import { exportActionPlanToPDF, ActionPlanPDFData } from '@/lib/pdf-export-service';
...
export async function GET(
  ...
  try {
    ...
    const completedObjectives = actionPlan.objectives?.filter(
    ...
    const pdfData: ActionPlanPDFData = {
      ...
      kpis: (actionPlan.kpis || []).map((kpi: any) => ({
        ...
        unit: kpi.unit,
        progress_percentage: kpi.target_value > 0 
          ? (kpi.current_value / kpi.target_value) * 100 
          : 0,
      })),
      qualitativeObjectives: (actionPlan.qualitative_objectives || []).map((obj: any) => ({
        description: obj.description,
        success_criteria: obj.success_criteria,
        completion_percentage: obj.completion_percentage || 0,
      })),
      progressUpdates: (actionPlan.progress_updates || []).map((update: any) => ({
